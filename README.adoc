...
Resources to process DocBook content. Features the DocBook schema (through https://central.sonatype.com/artifact/org.docbook/schemas-docbook/[dependency]) and the last version of DocBook XSL 1.
See example use.
...
https://central.sonatype.com/artifact/org.docbook/docbook-xslTNG[Release] 2.6.4 on Aug 2025 depends on jing, trang 20241231.
Embeds `org.xmlresolver.catalog.xml` and identical `org/docbook/xsltng/catalog.xml`, with entries such as 
`https://cdn.docbook.org/release/xsltng/2.6.0/xslt/VERSION.xsl` pointing to `classpath:org/docbook/xsltng/xslt/VERSION.xsl`.
“This catalog requires XML Resolver 2.0.0 or later”
